...
Comments are handled by a global variable integer commentCount, which serves as a counter. It is normally set to 0, meaning the file is currently in the initial state and not in a comment state. When /* is first reached, the state is changed to comment state and commentCount goes up by 1. Within the comment state, every time a /* is reached, commentCount goes up by 1. Whenever */ is reached, commentCount goes down by 1, and when commentCount reaches 0, the lexer is no longer in the comment state and returned back to the initial state.

Strings are a simpler case, there is a global variable stringState that is initially set to 0, and whenever \" is reached, it flips the state of stringState from 0 to 1 or 1 to 0. Special tokens within strings have and additional backslash to separate them from being recognized as those from the initial state.
...
Other implementation features we figured are worth noting are the inclusion of spaces within strings, as whitespaces within strings should contribute to the character location and line count.
